...
measuretype: Description of the laboratory or anthropometric test (height = height in cm, weight = weight in kg, glucose = glucose in mg/dl, hba1c = hemoglobin A1c in \%, chol = cholesterol in mg/dl)
...
chol.mod1 <- glm(T2DMcart.class~ firstage + factor(race) + gender + cholmean, data = data1, weights = w1)
summary(chol.mod1)

# Model using 2 step weights
data1$w.mod <- 1/data1$pmisschol.mod
chol.mod2 <- glm(T2DMcart.class~ firstage + factor(race) + gender + cholmean, data = data1, weights = w.mod)
summary(chol.mod2)
...
```{r, eval = FALSE, echo = TRUE, cache = TRUE, cache.lazy = FALSE}
## Summarize intensity of utilization using number of outpatient visits
hist(data1$numvisit)
...
# fit model
fmla.endo <- formula("Y ~ firstage + factor(race) + gender + dep")
