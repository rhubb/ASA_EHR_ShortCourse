...
```{r, eval = TRUE, echo = TRUE, cache = TRUE, cache.lazy = FALSE}
# for simplicity we will aggregate numeric measurements to the earlist, mean and maximum within this timeperiod
...
mu0 <- mean(inv.logit(data1$T2DMglm[data1$T2DMv == 0 & !is.na(data1$T2DMv)]))
mu1 <- mean(inv.logit(data1$T2DMglm[data1$T2DMv == 1 & !is.na(data1$T2DMv)]))
...
mod.prob.unadj <- glm(fmla.prob, data = data1, family = "binomial")
mod.prob.unadj$coef
```
