...
options(width = 400)
## To do:
## Fix probabilistic phenotypes example
## Add better comments and clean up code
## Add IPW missing data example -- may need to add an additional variable to data set like BP or cholesterol
## Take a look at Goldstein paper on conditioning on utilization and improve BMI example
## Create versions of output with and without code and results
```  
...
diag: ICD-9 (on or before 10/31/2015) or ICD-10 (after 10/31/2015) for primary diagnosis; a few diagnosis codes of interest are T2DM ICD-9 = "250.00", T2DM ICD-10 = "E11.9", T1DM ICD-9 = "250.01", T1DM ICD-10 = "E10.9"
...
#### Install R packages
- For these exercises you will need the _rpart_, _pROC_ _boot_, and _gee_ packages.
...
### Exercises
1\. **Data Quality Evaluation.** The first task in analysis of EHR data is data exploration and visualization to identify and resolve data errors. Using the measures data set, we will carry out a descriptive analysis. Are there any observations that seem likely to be errors? What are some techniques we can use to identify errors? What are some options for handling possibly erroneous data points once they have been identified?
...
encounter1 <- encounter[!duplicated(encounter$patientid),c("patientid","agemean","race","gender","T2DM","T1DM","endo","T2DMnum","dep")]
...
# fit model
fmla.endo <- formula("Y ~ agemean + factor(race) + gender + dep")
mod.MH <- mh.EM(fmla.endo, sens, spec, maxit = 100)
summary(mod.MH)
exp(mod.MH$coef)

# naive model for comparison
mod.cart <- glm(T2DMcart.class ~ agemean + factor(race) + gender + dep, data = data1, family = "binomial")
summary(mod.cart)
exp(mod.cart$coef)

# model based on validation data only
mod.valid <- glm(T2DMv ~ agemean + factor(race) + gender + dep, data = data1, family = "binomial")
summary(mod.valid)
exp(mod.valid$coef)
...
# fit model
fmla.prob <- formula("prob ~ agemean + factor(race) + gender + dep")
